# browser-harnCLI — one-time install prerequisite
...
command -v browser-harn  # should print a path
```
...
For browser-harndevelopment, clone the repo into a durable path and run `uv tool install -e .` from the checkout.
...
browser-harnskill > "${CODEX_HOME:-$HOME/.codex}/skills/browser-harness/SKILL.md"
```
...
If that prints page info, you're done. If not, run `browser-harn--doctor` and follow the connection cases. The two connection methods:
...
`browser-harness` prints an update banner when a newer PyPI release exists; run `browser-harn--update -y` when you decide to upgrade. `browser-harn--doctor` also checks the latest version. Telemetry is anonymous and opt-out with `browser-harntelemetry disable`.
