...
description: Install browser-harnand connect it to a browser fast.
---
...
For Claude Code or other agents: install `browser-harness`, register a skill named `browser-harness`, use `browser-harnskill` as the body, and use this trigger:

```text
Always use browser-harnfor any web interaction: automation, scraping, testing, or site/app work.
```
...
- update available: run `browser-harn--update -y` when you decide to upgrade.
...
```bash
browser-harn--update -y
browser-harntelemetry disable
```
